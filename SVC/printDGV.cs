...
namespace View
{
    ...
    {
        // Servirá para "guardar" o conteúdo da TextBox Cell e apresentar o texto através do DrawString
        private static StringFormat StrFormat;

        // Holds content of a Boolean Cell to write by DrawImage
        private static StringFormat StrFormatComboBox;

        // "Guarda" o conteúdo da célula referente ao botão 
        private static Button CellButton;

        // "Guarda" o conteúdo da célula referente à CheckBox
        private static CheckBox CellCheckBox;

        // "Guarda" o conteúdo da célula referente à ComboBox 
        private static ComboBox CellComboBox;

        // Soma o total de colunas
        private static int TotalWidth;

        // Posição da linha na impressão 
        private static int RowPos;

        // Indica se é necessário se é necessário uma nova página
        private static bool NewPage;

        // Número de páginas para imprimir
        private static int PageNo;

        // Coordenada esquerda das colunas
        ...
        // Altura da célula do DataGrid
        private static int CellHeight;

        // Número de linhas por página
        private static int RowsPerPage;

        // Tipo de objecto de documento de impressão para imprimir
        private static System.Drawing.Printing.PrintDocument printDoc = new System.Drawing.Printing.PrintDocument();

        // Cabeçalho das páginas
        private static string PrintTitle = "One Sistemas";

        // "Guarda" o objecto referente ao conteúdo da DataGridView
        private static DataGridView dgv;

        // As colunas seleccionadas pelo utilizador para imprimir.
        private static List<string> SelectedColumns = new List<string>();

        // Todas as colunas disponíveis noDataGridView
        private static List<string> AvailableColumns = new List<string>();

        // Este código não é necessário para este tutorial  
        ...
        public static void Print_DataGridView(DataGridView dgv1)
        {
            ...
            try
            {
                ...
                PrintOptions dlg = new PrintOptions(AvailableColumns, retrato);
                ...
                //Opção se for retrato ou paisagem
                printDoc.DefaultPageSettings.Landscape = dlg.ckPaisagem.Checked;
                //
                    ...
                    else
                    {
                        if (NewPage)
                        {
                            ...
                            e.Graphics.DrawString(PrintTitle,
                                                  ...
                                                  Brushes.Black,
                                                  e.MarginBounds.Left,
                                                  e.MarginBounds.Top - e.Graphics.MeasureString(PrintTitle, new Font(dgv.Font, FontStyle.Bold), e.MarginBounds.Width).Height - 13);
                            ...
                            e.Graphics.DrawString(s,
                                                  new Font(dgv.Font, FontStyle.Bold),
                                                  Brushes.Black,
                                                  e.MarginBounds.Left + (e.MarginBounds.Width - e.Graphics.MeasureString(s, new Font(dgv.Font, FontStyle.Bold), e.MarginBounds.Width).Width),
                                                  e.MarginBounds.Top - e.Graphics.MeasureString(PrintTitle, new Font(new Font(dgv.Font, FontStyle.Bold), FontStyle.Bold), e.MarginBounds.Width).Height - 13);
                            ...
                            foreach (DataGridViewColumn GridCol in dgv.Columns)
                            {
                                ...
                                e.Graphics.FillRectangle(new SolidBrush(Color.SteelBlue), new Rectangle((int)ColumnLefts[i], tmpTop, (int)ColumnWidths[i], HeaderHeight));

                                e.Graphics.DrawRectangle(Pens.Black, new Rectangle((int)ColumnLefts[i], tmpTop, (int)ColumnWidths[i], HeaderHeight));

                                e.Graphics.DrawString(GridCol.HeaderText, GridCol.InheritedStyle.Font, new SolidBrush(GridCol.InheritedStyle.ForeColor), new RectangleF((int)ColumnLefts[i], tmpTop, (int)ColumnWidths[i], HeaderHeight), StrFormat);
                        ...
                        foreach (DataGridViewCell Cel in GridRow.Cells)
                        {
                            ...
                            {
                                e.Graphics.DrawString(Cel.Value.ToString(), Cel.InheritedStyle.Font, new SolidBrush(Cel.InheritedStyle.ForeColor), new RectangleF((int)ColumnLefts[i], (float)tmpTop, (int)ColumnWidths[i], (float)CellHeight), StrFormat);
                            }
                            ...
                            else if (((Type)ColumnTypes[i]).Name == "DataGridViewCheckBoxColumn")
                            {
                                ...
                                CellCheckBox.DrawToBitmap(bmp, new Rectangle((int)((bmp.Width - CellCheckBox.Width) / 2), (int)((bmp.Height - CellCheckBox.Height) / 2), CellCheckBox.Width, CellCheckBox.Height));
                                e.Graphics.DrawImage(bmp, new Point((int)ColumnLefts[i], tmpTop));
                            }
                            ...
                            else if (((Type)ColumnTypes[i]).Name == "DataGridViewComboBoxColumn")
                            {
                                ...
                                e.Graphics.DrawImage(bmp, new Point((int)ColumnLefts[i], tmpTop));
                                e.Graphics.DrawString(Cel.Value.ToString(), Cel.InheritedStyle.Font, new SolidBrush(Cel.InheritedStyle.ForeColor), new RectangleF((int)ColumnLefts[i] + 1, tmpTop, (int)ColumnWidths[i] - 16, CellHeight), StrFormatComboBox);
                            }

                            // Desenha as margens das células
                            e.Graphics.DrawRectangle(Pens.Black, new Rectangle((int)ColumnLefts[i], tmpTop, (int)ColumnWidths[i], CellHeight));
        ...
        private static void DrawFooter(System.Drawing.Printing.PrintPageEventArgs e, int RowsPerPage)
        {
            ...
            string PageNum = PageNo.ToString() + " de " + Math.Ceiling((double)(cnt / RowsPerPage)).ToString();

            e.Graphics.DrawString(PageNum, dgv.Font, Brushes.Black, e.MarginBounds.Left + (e.MarginBounds.Width - e.Graphics.MeasureString(PageNum, dgv.Font, e.MarginBounds.Width).Width) / 2, e.MarginBounds.Top + e.MarginBounds.Height + 31);
        }

        public static bool retrato { get; set; }
    }
}
