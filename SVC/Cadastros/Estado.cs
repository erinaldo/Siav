...
namespace One.CADASTROS
{
    public partial class Estado : Form
    {
        ...
        EstadoBLL objEst = new EstadoBLL();
        ...
        public void carregaPropriedades()
        {
            try
            {
                if (txtCodigo.Text != "")
                {
                    objEst.est_codigo = int.Parse(txtCodigo.Text);
                ...
                if (txtDescricao.Text != "")
                    objEst.est_nome = txtDescricao.Text;
                else
                    throw new Exception("O campo descrição é de preenchimento obrigatório");
                txtSigla.Text = txtSigla.Text.Trim();

                if (txtSigla.Text != "")
                    objEst.est_sigla  = txtSigla.Text;
                else
                    throw new Exception("O campo sigla é de preenchimento obrigatório");
        ...
        public void carregaCampos()
        {
            try
            {
                if (objEst.est_codigo == 0)
                    ...
                    txtCodigo.Text = objEst.est_codigo.ToString();
                txtSigla.Text = objEst.est_sigla;
                txtDescricao.Text = objEst.est_nome;
        ...
        public void carregaGrid()
        {
            ...
            {
                DataTable tab = null;
                tab = objEst.localizarEmTudo(txtPesquisar.Text);
                gvPesquisa.DataSource = tab;
                gvPesquisa.Columns["est_codigo"].HeaderText = "Código";
                gvPesquisa.Columns["est_sigla"].HeaderText = "Sigla";
                gvPesquisa.Columns["est_nome"].HeaderText = "Nome";                
                gvPesquisa.Columns[0].Width = gvPesquisa.Width / 3;
                gvPesquisa.Columns[1].Width = gvPesquisa.Width / 3;
                gvPesquisa.Columns[2].Width = gvPesquisa.Width / 3-18;                
               // stiloGrid();
                gvPesquisa.ClearSelection();
            }
            catch (Exception)
        ...
        private void txtCodigo_Leave(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    objEst.limpar();
                    objEst.localizarLeave(cod.ToString(), "est_codigo");
                    if (objEst.est_codigo != 0)
                    {
                        carregaCampos();
                        txtCodigo.Enabled = false;
            ...
            try
            {
                if (e.RowIndex > -1 && e.RowIndex < gvPesquisa.Rows.Count)
                {
                    ...
                    if (cod != 0)
                    {
                        TabControl1.SelectedIndex = 0;
                        txtPesquisar.Text = "";
                        objEst.est_codigo = cod;
                        objEst.localizar(objEst.est_codigo.ToString(), "est_codigo");
                        txtCodigo.Text = objEst.est_codigo.ToString();
                        txtCodigo.Enabled = false;
                        carregaCampos();
        ...
        private void gvPesquisa_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (e.RowIndex > -1 && e.RowIndex < gvPesquisa.Rows.Count)
                {
                    objEst.localizar(gvPesquisa.Rows[e.RowIndex].Cells[0].Value.ToString(),"est_codigo");
                }
            }
            catch (Exception ex)
            {
        ...
        private void btSalvar_Click_1(object sender, EventArgs e)
        {
            try
            {
                ...
                if (objEst.est_codigo == 0) //Se o código for 0, significa que não é alteração, mas sim inserção 
                {
                    //Verificar se estado já existe
                    objEst.localizarLeave(objEst.est_nome, "est_nome"); //Pesquisa por descrição, na coluna descrição
                    if (objEst.est_codigo != 0) // se o código retornar um número acima de 0, significa que a categoria já está cadastrada
                        throw new Exception("Este estado já existe no sistema"); // Se cair no throw automaticamente os comandos de inserção são cancelados

                    objEst.localizarLeave(objEst.est_sigla, "est_sigla");
                    if (objEst.est_codigo != 0) // se o código retornar um número acima de 0, significa que a categoria já está cadastrada
                        throw new Exception("Esta sigla já existe no sistema"); // Se cair no throw automaticamente os comandos de inserção são cancelados

                    objEst.inserir();
                    objEst.limpar();
                    limpar();
                    MessageBox.Show("Estado incluído com sucesso", "Sucesso", MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1);
                }
                else //Alteração
                {
                    objEst.alterar();
                    objEst.limpar();
                    limpar();
                    MessageBox.Show("Estado alterado com sucesso", "Sucesso", MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1);
                }
            }
            catch (Exception ex)
            {
        ...
        private void btPrimeiro_Click_1(object sender, EventArgs e)
        {
            try
            {
                if (txtCodigo.Text != "")
                    ...
                    limpar();
                objEst.localizarPrimeiroUltimo("primeiro");
                if (objEst.est_codigo != 0)
                {
                    objEst.localizar(objEst.est_codigo.ToString(), "est_codigo");
                    carregaCampos();
        ...
        private void btAnterior_Click_1(object sender, EventArgs e)
        {
            try
            {
                if (txtCodigo.Text != "")
                    ...
                    limpar();
                if (txtCodigo.Text == "")
                    objEst.localizarProxAnterior("anterior", 0);
                else
                    objEst.localizarProxAnterior("anterior", int.Parse(txtCodigo.Text));
                if (objEst.est_codigo != 0)
                {
                    objEst.localizar(objEst.est_codigo.ToString(), "est_codigo");
                    carregaCampos();
        ...
        private void btProximo_Click_1(object sender, EventArgs e)
        {
            try
            {
                if (txtCodigo.Text != "")
                    ...
                    limpar();
                if (txtCodigo.Text == "")
                    objEst.localizarProxAnterior("proximo", 0);
                else
                    objEst.localizarProxAnterior("proximo", int.Parse(txtCodigo.Text));
                if (objEst.est_codigo != 0)
                {
                    objEst.localizar(objEst.est_codigo.ToString(), "est_codigo");
                    carregaCampos();
        ...
        private void btUltimo_Click_1(object sender, EventArgs e)
        {
            try
            {
                if (txtCodigo.Text != "")
                    ...
                    limpar();
                objEst.localizarPrimeiroUltimo("ultimo");
                if (objEst.est_codigo != 0)
                {
                    objEst.localizar(objEst.est_codigo.ToString(), "est_codigo");
                    carregaCampos();
