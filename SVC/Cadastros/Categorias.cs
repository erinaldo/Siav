...
namespace One.CADASTROS
{
    public partial class Categorias : Form
    {
        ...
        public void carregaPropriedades()
        {
            ...
            {
                if (txtCodigo.Text != "")
                    objCat.cat_codigo = int.Parse(txtCodigo.Text);
                else
                    objCat.cat_codigo = 0;
                txtDescricao.Text = txtDescricao.Text.Trim();
                if (txtDescricao.Text != "")
                    objCat.cat_descricao = txtDescricao.Text;
                else
        ...
        public void carregaCampos()
        {
            try
            {
                if (objCat.cat_codigo == 0)
                    txtCodigo.Text = "";
                else
                    txtCodigo.Text = objCat.cat_codigo.ToString();
                txtDescricao.Text = objCat.cat_descricao;
        ...
        private void btSalvar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (objCat.cat_codigo == 0) //Se o código for 0, significa que não é alteração, mas sim inserção 
                {
                    //Verificar se categoria já existe
                    objCat.localizarLeave(objCat.cat_descricao, "cat_descricao"); //Pesquisa por descrição, na coluna descrição
                    if (objCat.cat_codigo != 0) // se o código retornar um número acima de 0, significa que a categoria já está cadastrada
                        throw new Exception("Esta categoria já existe no sistema"); // Se cair no throw automaticamente os comandos de inserção são cancelados
                    
                    objCat.inserir();
                    objCat.limpar();
                    limpar();
                    MessageBox.Show("Categoria incluída com sucesso", "Sucesso", MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1);
                }
                else //Alteração
                {
                    objCat.alterar();
            ...
            try
            {
                if (objCat.cat_codigo != 0)
                {
                    if (MessageBox.Show("Deseja realmente excluir?", "Excluir", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) == System.Windows.Forms.DialogResult.Yes)
                    {
                        ...
                        limpar();
                        carregaGrid();
                        MessageBox.Show("Categoria excluída com sucesso", "Sucesso", MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1);
                    }
                }
                else
                    MessageBox.Show("Favor selecionar uma categoria na aba 'Pesquisar', ou escolher um código válido para poder excluir", "Excluir", MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1);                    
            }
            catch (SqlException)
            {
                MessageBox.Show("Esta categoria está vinculada a alguma movimentação do sistema e não pode ser excluída", "Erro", MessageBoxButtons.OK, MessageBoxIcon.Error, MessageBoxDefaultButton.Button1);
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message, "Erro", MessageBoxButtons.OK, MessageBoxIcon.Error, MessageBoxDefaultButton.Button1);
            ...
            try
            {
                if (e.RowIndex > -1 && e.RowIndex < gvPesquisa.Rows.Count)
                {
                    ...
                    if (cod != 0)
                    {
                        ...
                        txtPesquisar.Text = "";
                        objCat.cat_codigo = cod;
                        objCat.localizar(objCat.cat_codigo.ToString(),"cat_codigo");
                        txtCodigo.Text = objCat.cat_codigo.ToString();
        ...
        private void txtCodigo_Leave(object sender, EventArgs e)
        {
            try
            {
                 // Localizar usuário
                    ...
                    {
                        objCat.limpar();
                        objCat.localizarLeave(catcod.ToString(), "cat_codigo");
                        if (objCat.cat_codigo != 0)
                        {
                            carregaCampos();
                            txtCodigo.Enabled = false;
        ...
        private void gvPesquisa_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (e.RowIndex > -1 && e.RowIndex < gvPesquisa.Rows.Count)
                {
                    objCat.localizar(gvPesquisa.Rows[e.RowIndex].Cells[0].Value.ToString(), "cat_codigo");
                }
            }
            catch (Exception ex)
            {
