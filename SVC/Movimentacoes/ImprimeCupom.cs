...
namespace ImprimeCupom
{
	...
	public class ImprimeTexto
	{
		...
		public bool Inicio(string sPortaInicio)
		{
            ...
            else if (sPortaInicio.Substring(0, 3) == "COM")
            {
                //if (sPortaInicio == "LPT1")
                //{
                //    sPortaInicio = "LPT3";
                //}
                sPorta = sPortaInicio;
                sPortaInicio = "COM-" + DateTime.Now.Hour.ToString("00") + DateTime.Now.Minute.ToString("00") + DateTime.Now.Second.ToString("00") + ".TXT";
                GeraArquivoLPT = sPortaInicio;
                fileWriter = new StreamWriter(sPortaInicio);
                lOK = true;
                //hPort = CreateFileA(sPorta, GENERIC_WRITE, FILE_SHARE_WRITE, 0, OPEN_EXISTING, 0, 0);
            ...
            try
            {
                if (lOK)
                {
                    ...
                    if (GeraArquivoLPT != String.Empty)
                    {
                        try
                        {
                            System.IO.File.Copy(GeraArquivoLPT, "C:\\impressora\\lpt.txt");
                            File.Delete(GeraArquivoLPT);
                            GeraArquivoLPT = "";
                        }
                        catch
                        {
                            File.Delete("C:\\impressora\\lpt.txt");
                            System.IO.File.Copy(GeraArquivoLPT, "C:\\impressora\\lpt.txt");
                            File.Delete(GeraArquivoLPT);
                            GeraArquivoLPT = "";
                        }
